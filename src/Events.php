...
class Events
{
    const HTTP_RETRIEVE_ARGS = array(
        'headers' => array(
            'User-Agent' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:105.0) Gecko/20100101 Firefox/105.0 +CCFoodTrucks'
        ),
        'timeout' => 601
    );
    ...
    {
        if (!get_option('food_truck_checkbox')) {
            return '';
        }

        $sheet_id = get_option('food_truck_sheet_id');
        $sheet_name = get_option('food_truck_sheet_name');
        $google_api_key = get_option('food_truck_google_api_key');
    ...
    {
        return '<div id="food-truck-container"></div>';
    }
